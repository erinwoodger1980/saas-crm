  ...
  try {
    ...
    const doParse = async () => {
      ...
      for (const f of filesToParse) {
        ...
        const { normalized, normalizedQuote, lines, supplier, currency } = normaliseParsedQuote(selected.parsed);
        for (const ln of lines) {
          ...
          const currencySource =
            resolveCurrency(ln) || currency || selected.currency || quote.currency || "GBP";
          const lineCurrency = String(currencySource || "GBP").toUpperCase();
          const lineSupplier =
            resolveSupplier(ln) || supplier || selected.supplier || (quote as any).supplier || undefined;
