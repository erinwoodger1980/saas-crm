  ...
  try {
    ...
    const doParse = async () => {
      ...
      for (const f of filesToParse) {
        ...
        const normalizedParsedSnapshot = parsedSummary?.normalized;
        const normalizedQuoteSnapshot = parsedSummary?.normalizedQuote;
        const supplierFromParse = parsedSummary?.supplier;
        const currencyFromParse = parsedSummary?.currency;
        const candidateLines = parsedSummary?.lines;
        const lines = Array.isArray(candidateLines) ? candidateLines : [];
