        ...
        for (const m of messages) {
          try {
            ...
            try {
              await prisma.emailMessage.upsert({
                where: { tenantId_provider_messageId: { tenantId, provider: "gmail", messageId: m.id } },
                update: { threadId: thread.id, fromEmail: fromAddr, toEmail: allRcpts.length ? allRcpts.join(", ") : null, subject: subject || null, snippet: snippet || null, bodyText: storedBodyText, sentAt: msg.internalDate ? new Date(Number(msg.internalDate)) : new Date(), leadId: leadId || null },
                create: { tenantId, provider: "gmail", messageId: m.id, threadId: thread.id, direction: "inbound", fromEmail: fromAddr, toEmail: allRcpts.length ? allRcpts.join(", ") : null, subject: subject || null, snippet: snippet || null, bodyText: storedBodyText, sentAt: msg.internalDate ? new Date(Number(msg.internalDate)) : new Date(), leadId: leadId || null },
              });
            } catch {}
