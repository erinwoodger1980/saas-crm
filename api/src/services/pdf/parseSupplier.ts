...
async function rasterizePageToPng(filePath: string, pageNum: number, scale = 2): Promise<Buffer> {
  ...
  const puppeteerModuleName: string = "puppeteer-core";
  const puppeteerImport = await import(puppeteerModuleName).catch((err) => {
    throw new Error(`puppeteer-core is required for OCR rasterization: ${err}`);
  });
...
export async function parseSupplierPdf(filePath: string, opts: Options = {}): Promise<Parsed> {
  ...
  for (const page of pages) {
    ...
    try {
      ...
      console.warn(`[parseSupplier] OCR fallback failed for page ${page.page}`, err);
      results.push(page.raw);
    }
  }
