...
export async function seedConfiguredProductStarter({ tenantId }: StarterData) {
  ...
  const entranceQuestions = questions.filter(q => 
    ['width', 'height', 'timberGroup', 'finish', 'glazingType', 'hardwarePack', 'colourInside', 'colourOutside'].includes(q.attributeCode)
  );
  
  await Promise.all(
    entranceQuestions.map((question, idx) => 
      prisma.questionSetQuestion.create({
        data: {
