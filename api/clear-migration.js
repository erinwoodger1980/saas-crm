...
// Load api/.env if DATABASE_URL isn't already set.
...
if (!process.env.DATABASE_URL) {
  console.error('❌ DATABASE_URL not set. Set it in api/.env or export it before running this script.');
  process.exit(1);
}
...
async function clearFailedMigration() {
  ...
  try {
    ...
    if (migrations.length === 0) {
      ...
      console.log('Found migration:', migrations[0]);
      ...
      console.log(`✅ Successfully deleted ${result} migration record(s)`);
    }
