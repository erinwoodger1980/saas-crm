    ...
    >
      <div className="relative flex h-[min(88vh,calc(100vh-3rem))] w-[min(1000px,92vw)] max-h-[88vh] flex-col overflow-hidden rounded-3xl border border-white/30 bg-white/85 shadow-[0_32px_70px_-35px_rgba(30,64,175,0.45)] backdrop-blur-xl">
        ...
        <div className="flex-1 overflow-y-auto">
          ...
          {/* TASKS & FOLLOW-UPS STAGE - Unified View with AI Follow-ups */}
          {currentStage === "tasks" && (
            <div className="p-4 sm:p-6 bg-gradient-to-br from-white via-purple-50/70 to-blue-50/60 min-h-[60vh]">
              <div className="max-w-6xl mx-auto">
                ...
                <div className="mb-6">
                  <div className="flex items-start justify-between gap-4 mb-2">
                    <div>
                      ...
                      <p className="text-sm text-slate-600 mt-1">All tasks and action items for this lead</p>
                    </div>
                    <button
                      type="button"
                  ...
                  {showTaskComposer && (
                    <form
                      className="bg-slate-50 rounded-xl p-4 space-y-4 mb-6 border border-slate-200"
                      onSubmit={(e) => {
                        e.preventDefault();
                        createManualTask();
                      }}
                    >
                      ...
                      <label className="block text-sm font-semibold text-slate-600">
                        ...
                        <input
                          ...
                          className="mt-1 w-full rounded-lg border border-slate-200 bg-white px-3 py-2 text-sm focus:border-sky-300 focus:outline-none focus:ring-2 focus:ring-sky-200"
                      ...
                      <label className="block text-sm font-semibold text-slate-600">
                        ...
                        <textarea
                          ...
                          className="mt-1 w-full rounded-lg border border-slate-200 bg-white px-3 py-2 text-sm focus:border-sky-300 focus:outline-none focus:ring-2 focus:ring-sky-200 min-h-[72px]"
                      ...
                      <button
                        type="submit"
                        className="w-full rounded-lg bg-slate-900 px-4 py-2.5 text-sm font-semibold text-white hover:bg-slate-800 transition-colors"
                      >
                        Create Task
                      </button>
                    </form>
                ...
                {pendingTasks.length === 0 ? (
                  <div className="rounded-xl border-2 border-dashed border-slate-200 bg-slate-50 p-8 text-center mb-8">
                  ...
                  <div className="space-y-3 mb-8">
                    {pendingTasks.map((task) => {
                      const isAITask = (task as any).taskType === 'FOLLOW_UP' || (task as any).meta?.aiDraft;
                      const taskMeta = (task as any).meta || {};
                      const isReviewEnquiry = task.title.toLowerCase().includes('review enquiry');
                      const trigger = taskMeta.trigger || '';
                      
                      let icon = isAITask ? '✨' : taskMeta.type === 'email_followup' ? '📧' : taskMeta.type === 'phone_followup' ? '📞' : '📋';
                      let borderColor = isAITask ? 'border-indigo-300' : taskMeta.type === 'email_followup' ? 'border-blue-200' : 'border-slate-200';
                      let bgColor = isAITask ? 'bg-gradient-to-r from-indigo-50 to-purple-50' : taskMeta.type === 'email_followup' ? 'bg-blue-50' : 'bg-white';
                      
                      return (
                        <div key={task.id} className={`rounded-xl border-2 ${borderColor} ${bgColor} hover:shadow-lg transition-all`}>
                          <div className="p-4">
                            <div className="flex items-start justify-between gap-3 mb-3" onClick={() => setSelectedFollowUpTask(task as Task)} style={{cursor: 'pointer'}}>
                              <div className="flex items-start gap-3 flex-1">
                                <span className="text-2xl">{icon}</span>
                                <div className="flex-1 min-w-0">
                                  <h4 className="text-base font-semibold text-slate-900 mb-1">{task.title}</h4>
                                  {isAITask && taskMeta.aiDraft && (
                                    <div className="text-sm text-indigo-600 font-medium mb-1">✓ AI Draft Ready • {Math.round((taskMeta.aiDraft.confidence || 0) * 100)}% Confidence</div>
                                  )}
                                  {task.description && <p className="text-sm text-slate-600 line-clamp-2 mb-2">{task.description}</p>}
                                  <div className="flex items-center gap-3 text-xs text-slate-500">
                                    {task.dueAt && <span>Due: {new Date(task.dueAt).toLocaleDateString()}</span>}
                                  </div>
                                </div>
                              </div>
                              <div className="text-sm font-medium text-indigo-600 whitespace-nowrap">{isAITask ? 'Review →' : 'View →'}</div>
                            </div>
                            
                            {/* Contextual Actions */}
                            <div className="flex gap-2 pt-3 border-t border-slate-200" onClick={(e) => e.stopPropagation()}>
                              {isReviewEnquiry ? (
                                ...
                                <>
                                  <button
                                    ...
                                    onClick={async () => {
                                      try {
                                        const preview = await apiFetch<any>(`/tasks/${task.id}/actions/accept-enquiry/preview`, {
                                          method: 'POST',
                                          headers: authHeaders,
                                        });
                                        setEmailPreview({
                                        ...
                                        toast('Failed to generate preview');
                                      }
                                    }}
                                  >
                                    ✓ Accept
                                  ...
                                  <button
                                    className="flex-1 px-3 py-2 text-sm font-medium rounded-lg bg-orange-600 text-white hover:bg-orange-700 transition-colors"
                                    onClick={async () => {
                                      try {
                                        const preview = await apiFetch<any>(`/tasks/${task.id}/actions/decline-enquiry/preview`, {
                                          method: 'POST',
                                          headers: authHeaders,
                                        });
                                        setEmailPreview({
                                  ...
                                  <button
                                    className="flex-1 px-3 py-2 text-sm font-medium rounded-lg border border-red-400 text-red-700 hover:bg-red-50 transition-colors"
                                    onClick={async () => {
                                      if (confirm('Reject as not a real enquiry? This will mark the lead as rejected and provide feedback to the ML system.')) {
                                        try {
                                          await apiFetch(`/tasks/${task.id}/actions/reject-enquiry`, {
                                            method: 'POST',
                                            headers: authHeaders,
                                          });
                                          await refreshActivity();
                                          ...
                                          toast('Failed to reject enquiry');
                                        }
                                      }
                                    }}
                                  >
                                ...
                                // Follow-up tasks: Send / Skip / Details
                                <>
                                  <button
                                    className="flex-1 px-3 py-2 text-sm font-medium rounded-lg bg-indigo-600 text-white hover:bg-indigo-700 transition-colors"
                                    onClick={() => setSelectedFollowUpTask(task as Task)}
                                  >
                                    📧 Send Email
                                  </button>
                                  <button
                                    className="flex-1 px-3 py-2 text-sm font-medium rounded-lg border border-slate-300 text-slate-700 hover:bg-slate-50 transition-colors"
                                    onClick={async () => {
                                  ...
                                  <button
                                    className="px-3 py-2 text-sm font-medium rounded-lg border border-slate-300 text-slate-700 hover:bg-slate-50 transition-colors"
                                    onClick={() => setSelectedFollowUpTask(task as Task)}
                                  >
                                    👁️
                                  </button>
                                ...
                                // Generic tasks: Complete / Defer / Details
                                <>
                                  <button
                                    className="flex-1 px-3 py-2 text-sm font-medium rounded-lg bg-green-600 text-white hover:bg-green-700 transition-colors"
                                    onClick={async () => {
                                  ...
                                  <button
                                    className="flex-1 px-3 py-2 text-sm font-medium rounded-lg border border-slate-300 text-slate-700 hover:bg-slate-50 transition-colors"
                                    onClick={async () => {
                                      const days = prompt('Defer by how many days?', '1');
                                      if (days) {
                                        ...
                                        await apiFetch(`/tasks/${task.id}`, {
                                          method: 'PATCH',
                                          headers: authHeaders,
                                          json: { dueAt: newDue.toISOString() }
                                        });
                                        await refreshActivity();
                                        toast(`Task deferred by ${days} days`);
                                      }
                                    }}
                                  >
                                    ⏰ Defer
                ...
                {completedTasks.length > 0 && (
                  <div className="mb-8">
                    <h3 className="text-lg font-semibold text-slate-900 mb-3">Completed Tasks</h3>
                    <div className="bg-slate-50 rounded-lg border border-slate-200 p-4 space-y-2">
                      {completedTasks.map((task) => {
                        const meta = (task as any).meta || {};
                        const taskWithDate = task as any;
                        const completedAt = meta.completedAt ? new Date(meta.completedAt) : taskWithDate.updatedAt ? new Date(taskWithDate.updatedAt) : null;
                        return (
                          <div key={task.id} className="flex items-center justify-between gap-3 text-sm py-1">
                ...
                {/* Legacy section removed - all tasks now shown above */}
                {false && pendingFollowUpTasks.filter(t => t.type === 'task' && ((t as any).taskType === 'FOLLOW_UP' || (t as any).meta?.aiDraft)).length > 0 && (
                  <div className="mb-8 rounded-2xl border-2 border-indigo-300 bg-gradient-to-br from-indigo-50 to-purple-50 p-6 shadow-xl">
                    <div className="flex items-center gap-4 mb-5">
                      ...
                      <div className="flex-1">
                        <h2 className="text-xl font-bold text-slate-900">AI-Powered Follow-ups</h2>
                        <p className="text-sm text-slate-600 mt-1">Smart email drafts ready to review and send</p>
                      </div>
                      <div className="px-4 py-2 rounded-full bg-indigo-600 text-white text-sm font-semibold">
                        {pendingFollowUpTasks.filter(t => t.type === 'task' && ((t as any).taskType === 'FOLLOW_UP' || (t as any).meta?.aiDraft)).length} ready
                      </div>
                    </div>
                    
                    <div className="space-y-3">
                      {pendingFollowUpTasks.filter(t => t.type === 'task' && ((t as any).taskType === 'FOLLOW_UP' || (t as any).meta?.aiDraft)).map((event) => {
                        if (event.type !== 'task') return null;
                        return (
                          <div key={event.id} className="rounded-xl border-2 border-indigo-200 bg-white hover:shadow-lg transition-all">
                            <Button
                              variant="ghost"
                              className="w-full p-5 text-left flex items-center justify-between hover:bg-indigo-50/50"
                                ...
                                <div className="flex-1">
                                  ...
                                  {(event as any).meta?.aiDraft && (
                                    <div className="text-sm text-indigo-600 font-medium mt-1">
                                      ✓ AI Draft Ready • {Math.round(((event as any).meta.aiDraft.confidence || 0) * 100)}% Confidence
                                    </div>
                                  )}
                                  {event.description && (
                                    <div className="text-xs text-slate-600 mt-2 line-clamp-2">{event.description}</div>
                                  )}
                                </div>
                              </div>
                              <div className="text-right ml-4">
                                <div className="text-xs text-slate-500 mb-1">
                                  {event.dueAt ? new Date(event.dueAt).toLocaleDateString() : "Due soon"}
                                </div>
